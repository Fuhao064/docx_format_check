  ...
  <div class="flex flex-col h-screen">
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="flex-1 flex flex-col overflow-hidden p-4" :class="isDarkMode ? 'bg-zinc-900' : 'bg-gray-50'">
        <!-- 顶部状态/结果区域 - 可折叠 -->
        <div class="mb-4 rounded-lg border transition-all duration-300 overflow-hidden" :class="[
          ...
          <div class="flex items-center justify-between p-2 cursor-pointer" @click="toggleStatusArea">
            ...
            <button class="p-1 rounded-md transition-colors"
              ...
              <svg v-if="statusAreaExpanded" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
          ...
          <div class="p-4 overflow-y-auto max-h-[450px] scrollbar-thin">
            ...
            <div v-else-if="currentStep === 1" class="flex flex-col items-center justify-center py-8">
              ...
              <div class="flex gap-4">
                <!-- 上传格式文档按钮 -->
                <button @click="triggerFormatFileUpload"
                  class="flex items-center justify-center gap-2 px-4 py-2 rounded-md transition-colors"
                  :class="isDarkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div v-else-if="currentStep >= 2" class="flex flex-col">
              ...
              <div class="flex flex-col space-y-4 mb-8">
                <div v-for="(step, index) in processingSteps" :key="index" class="flex items-center p-3 rounded-lg"
                  :class="[
                  ...
                  <div class="w-8 h-8 rounded-full flex items-center justify-center mr-3" :class="[
                    ...
                    <svg v-if="step.status === 'completed'" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round">
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                    <svg v-else-if="step.status === 'in_progress'" xmlns="http://www.w3.org/2000/svg" width="16"
                      height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                      stroke-linecap="round" stroke-linejoin="round">
                      <circle cx="12" cy="12" r="10"></circle>
                      <polyline points="12 6 12 12 16 14"></polyline>
                    </svg>
                    <svg v-else-if="step.status === 'error'" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round">
                  ...
                  <div class="flex-1">
                    ...
                    <p class="text-sm" :class="isDarkMode ? 'text-zinc-400' : 'text-gray-700'">{{ step.description }}
                    </p>
                  </div>
                  <div class="ml-4">
              ...
              <div v-if="formatErrors.length > 0" class="mt-4 mb-8">
                <h3 class="text-lg font-medium mb-3 flex items-center"
                  :class="isDarkMode ? 'text-zinc-100' : 'text-gray-900'">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex-1 flex flex-col rounded-lg border overflow-hidden"
          ...
          <div class="flex border-b" :class="isDarkMode ? 'border-zinc-700' : 'border-gray-200'">
            <button v-for="agent in agents" :key="agent.id" @click="selectAgent(agent.id)"
              class="px-4 py-2 flex items-center gap-2 transition-colors" :class="[
            ...
            <!-- 协作模式按钮 -->
            <button @click="enableCollaborationMode" class="px-4 py-2 flex items-center gap-2 ml-auto transition-colors"
              :class="[
                collaborationMode
                  ? isDarkMode
          ...
          <div class="flex-1 overflow-hidden">
            <ChatInterface :agent-id="currentAgentId" :collaboration-mode="collaborationMode"
              :uploaded-file="uploadedFile" :format-file="uploadedFormatFile" :format-errors="formatErrors" />
          </div>
        </div>
      </div>
      ...
      <div :class="[
        ...
        isDarkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-gray-200',
        showDocPreview ? docPreviewExpanded ? 'w-full' : 'w-2/5' : 'w-16'
      ]">
        <div class="flex items-center justify-between p-2 border-b"
          ...
          <h3 class="font-medium truncate" :class="isDarkMode ? 'text-zinc-100' : 'text-gray-900'">
            文档预览
            <span v-if="uploadedFile" class="text-sm font-normal ml-2"
              :class="isDarkMode ? 'text-zinc-400' : 'text-gray-600'">
              {{ uploadedFileName }}
            </span>
          </h3>
          <div class="flex items-center">
            <!-- 全屏预览按钮 -->
            <button @click="toggleDocPreviewExpand" class="p-1 rounded-md transition-colors mr-1"
              :class="isDarkMode ? 'text-zinc-400 hover:text-zinc-100' : 'text-gray-500 hover:text-gray-700'">
              <svg v-if="docPreviewExpanded" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round">
            ...
            <button @click="toggleDocPreview" class="p-1 rounded-md transition-colors"
              :class="isDarkMode ? 'text-zinc-400 hover:text-zinc-100' : 'text-gray-500 hover:text-gray-700'">
              <svg v-if="showDocPreview" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
        ...
        <!-- 添加 v-if="showDocPreview" 以控制内容区域显示 -->
        <div v-if="showDocPreview" class="h-[calc(100%-40px)]">
        ...
        <div v-else class="h-[calc(100%-40px)] flex items-center justify-center"
          :class="isDarkMode ? 'text-transparent' : 'text-transparent'">
          <!-- 折叠时内容区域为空，文字透明以避免空间占用 -->
        </div>
      </div>
    </div>
    ...
    <!-- 隐藏的格式文件上传输入 -->
    <input type="file" ref="formatFileInput" @change="handleFormatFileUpload" accept=".docx,.json" class="hidden" />
...
const conversationId = ref(route.query.chatId || uuidv4())
...
function restoreState() {
  const state = JSON.parse(sessionStorage.getItem(`chatState-${conversationId.value}`) || '{}')
  uploadedFile.value = state.uploadedFile || null
  uploadedFormatFile.value = state.uploadedFormatFile || null
  uploadedFileName.value = state.uploadedFileName || ''
  uploadedFormatFileName.value = state.uploadedFormatFileName || ''
  currentStep.value = state.currentStep || 0
  processingSteps.value = state.processingSteps || processingSteps.value
  formatErrors.value = state.formatErrors || []
  processingComplete.value = state.processingComplete || false
  showDocPreview.value = state.showDocPreview || true
}

function saveState() {
  const state = {
    uploadedFile: uploadedFile.value,
    uploadedFormatFile: uploadedFormatFile.value,
    uploadedFileName: uploadedFileName.value,
    uploadedFormatFileName: uploadedFormatFileName.value,
    currentStep: currentStep.value,
    processingSteps: processingSteps.value,
    formatErrors: formatErrors.value,
    processingComplete: processingComplete.value,
    showDocPreview: showDocPreview.value
  }
  sessionStorage.setItem(`chatState-${conversationId.value}`, JSON.stringify(state))
}
...
async function handleFileUpload(event) {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      // Update sidebar with document name
      updateChatHistory(file.name, 'bg-green-500/20 text-green-400')
      saveState()
