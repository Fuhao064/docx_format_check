...
def main():
    ...
    for para in manager.paragraphs:
        if para.type.value == 'figures':
            figures_count += 1
            print(f"\u56fe片段落: {para.content}")
            if 'image_path' in para.meta:
                print(f"\u56fe片路径: {para.meta['image_path']}")
            if 'binary_data' in para.meta:
                print("\u5305含二进制数据")
        elif para.type.value == 'tables':
            tables_count += 1
            print(f"\u8868格段落: {para.content}")
            if 'table_data' in para.meta:
                print(f"\u8868格数据行数: {len(para.meta['table_data'])}")
