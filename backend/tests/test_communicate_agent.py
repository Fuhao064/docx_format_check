...
class TestCommunicateAgent(unittest.TestCase):
    ...
    def test_get_response_format_agent(self, mock_format_agent_class):
        ...
        # 添加doc_content参数
        result = self.agent.get_response("请解析这个格式要求", "这是文档内容")

        mock_format_agent_class.assert_called_once_with(self.agent.model)
        # 由于我们修改了实现，现在调用的是process方法而不是parse_format
        mock_format_agent.process.assert_called_once()
    ...
    def test_get_response_editor_agent(self, mock_editor_agent_class):
        ...
        # 添加doc_content参数
        result = self.agent.get_response("请为这个图片生成标题", "这是文档内容")

        mock_editor_agent_class.assert_called_once_with(self.agent.model)
        mock_editor_agent.get_image_caption.assert_called_once()
    ...
    def test_get_response_advice_agent(self, mock_advice_agent_class):
        ...
        # 添加doc_content参数
        doc_content = "这是文档内容"
        result = self.agent.get_response("请给我一些建议", doc_content)

        mock_advice_agent_class.assert_called_once_with(self.agent.model)
        # 确保provide_advice方法被调用，并且传入了doc_content
        mock_advice_agent.provide_advice.assert_called_once_with(doc_content)
    ...
    def test_chat_function(self):
        ...
        # 测试带文档内容的情况
        result = self.agent.chat("你好", "这是文档内容")
        self.assertEqual(result, "这是一个回复")
