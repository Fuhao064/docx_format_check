...
def _save_uploaded_document(file_storage) -> Dict[str, Any]:
    ...
    if _safe_ext(filename) != "docx":
        raise ValueError("Only .docx files are supported")
    unique_name = f"{os.path.splitext(filename)[0]}_{int(time.time())}_{uuid4().hex[:8]}.docx"
    path = os.path.join(UPLOAD_FOLDER, unique_name)
    file_storage.save(path)
    record = {"document_id": _new_id("doc"), "doc_path": path, "original_filename": filename, "created_at": datetime.utcnow()}
    with registry_lock:
