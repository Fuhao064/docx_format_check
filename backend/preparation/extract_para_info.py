...
try:
    ...
    def add_media_to_manager(manager, doc_path):
        logger.warning("未找到 extract_media 模块，无法提取图片和表格")
        return manager
...
def extract_para_format_info_from_paragraph_fromat(para)-> dict:
    ...
    try:
        ...
        logger.error(f"解析段落格式信息出错: {str(e)}")
        # 发生错误时，使用默认值
                        ...
                        if sz_element is not None:
                            ...
                            if sz_val:
                                try:
                                    ...
                                    logger.debug(f"从docDefaults提取到默认字体大小: {font_size}pt")
                                ...
                                try:
                                    ...
                                    for key in default_sizes:
                                        if default_sizes[key] is None:
                                            default_sizes[key] = font_size
                                    logger.debug(f"从docDefaults提取到默认复杂脚本字体大小: {font_size}pt")
                                except (ValueError, TypeError):
                                    pass
                        ...
                        if sz_element is not None and '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}val' in sz_element.attrib:
                            ...
                            try:
                                ...
                                if style_type in default_sizes:
                                    ...
                                    logger.debug(f"从styles.xml提取到{style_type}类型的默认字体大小: {font_size}pt")
                            except (ValueError, TypeError):
                                pass
                        ...
                        if style_type in default_sizes and default_sizes[style_type] is None:
                            ...
                            if szCs_element is not None and '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}val' in szCs_element.attrib:
                                ...
                                try:
                                    font_size = float(szCs_val) / 2
                                    default_sizes[style_type] = font_size
                                    logger.debug(f"从styles.xml提取到{style_type}类型的默认复杂脚本字体大小: {font_size}pt")
                                except (ValueError, TypeError):
                                    pass
                                ...
                                if val_attr == 'Normal':
                                    ...
                                    if sz_element is not None and '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}val' in sz_element.attrib:
                                        ...
                                        try:
                                            ...
                                            logger.debug(f"从styles.xml的Normal样式中提取到默认字体大小: {font_size}pt")
                                        except (ValueError, TypeError):
                                            pass
                                    ...
                                    if default_sizes['paragraph'] is None:
                                        ...
                                        if szCs_element is not None and '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}val' in szCs_element.attrib:
                                            ...
                                            try:
                                                font_size = float(szCs_val) / 2
                                                default_sizes['paragraph'] = font_size
                                                logger.debug(f"从styles.xml的Normal样式中提取到默认复杂脚本字体大小: {font_size}pt")
                                            except (ValueError, TypeError):
                                                pass
                                    break

                logger.debug(f"从styles.xml提取的默认字体大小: {default_sizes}")
    except Exception as e:
        logger.error(f"从styles.xml提取字体大小信息时出错: {str(e)}")
...
def extract_para_format_info(doc_path, manager: ParagraphManager):
    ...
    logger.info(f"从文档中提取到 {len(styles_info)} 个段落样式")

    # 从theme1.xml中提取字体信息
    theme_fonts = extract_font_from_theme(doc_path)
    logger.debug(f"从theme1.xml提取的字体信息：中文字体={theme_fonts['zh_family']}, 英文字体={theme_fonts['en_family']}")

    # 从styles.xml中提取默认字体大小信息
    default_font_sizes = extract_default_font_size_from_styles(doc_path)
    logger.debug(f"从styles.xml提取的默认字体大小：{default_font_sizes}")

    # 提取图片和表格信息（存储到manager中，不作为独立段落添加）
    # 注意：图片和表格信息存储在 manager.figures 和 manager.tables 中
    # 这些信息会通过 manager.to_dict() 中的 extra_info 返回
    from preparation.extract_media import extract_images_from_docx, extract_tables_from_docx

    # 只提取信息，不添加为独立段落
    manager.figures = extract_images_from_docx(doc_path)
    manager.tables = extract_tables_from_docx(doc_path)
    logger.info(f"已从文档中提取 {len(manager.figures)} 个图片和 {len(manager.tables)} 个表格信息")
    ...
    for para in processed_paras:
        ...
        if processed_count % 10 == 0 or processed_count == total_paras:
            logger.debug(f"处理进度：{processed_count}/{total_paras} ({int(processed_count/total_paras*100)}%)")
        ...
        logger.debug(f"  样式名称: {para.style.name}")
        ...
        logger.debug(f"  从runs中提取的字体信息: {fonts_from_runs}")
        ...
        if is_font_dict_empty(fonts_from_xml) and style_xml_data:
            ...
            logger.debug(f"  从样式XML中提取的字体信息: {fonts_from_xml}")
        else:
            logger.debug(f"  从段落XML中提取的字体信息: {fonts_from_xml}")
        ...
        if not fonts['zh_family'] or 'Unknown' in fonts['zh_family']:
            if theme_fonts['zh_family']:
                ...
                if fonts['zh_family'] != original_zh_fonts:
                    logger.debug(f"  从theme1.xml补充中文字体: {theme_fonts['zh_family']}")
        ...
        if not fonts['en_family'] or 'Unknown' in fonts['en_family']:
            if theme_fonts['en_family']:
                ...
                if 'Unknown' in fonts['en_family']:
                    ...
                    logger.debug(f"  移除了'Unknown'英文字体标记")
                ...
                if fonts['en_family'] != original_en_fonts:
                    logger.debug(f"  从theme1.xml补充英文字体: {theme_fonts['en_family']}")
        ...
        if not fonts['size'] or 'Unknown' in fonts['size']:
            ...
            if default_size is not None:
                ...
                if fonts['size'] != original_sizes:
                    logger.debug(f"  从styles.xml补充默认字体大小: {default_size}pt")

        # 最终的字体信息汇总
        logger.debug(f"  最终字体信息: zh_family={fonts['zh_family']}, en_family={fonts['en_family']}, "
        ...
        if not isinstance(para_type, ParsedParaType):
            logger.warning(f"  警告：段落类型不是ParsedParaType枚举，而是{type(para_type)}，将使用BODY类型")
            para_type = ParsedParaType.BODY
...
def extract_font_info_from_runs(runs: list) -> dict:
    ...
    if run_bold_states:
        ...
        if bold_text_length > 0:
            fonts['bold'].add(True)
            logger.debug(f"  段落中有加粗文本，加粗文本长度: {bold_text_length}, 总文本长度: {total_text_length}")
        else:
            # 确保加粗状态为空集合，并添加False值
            fonts['bold'] = {False}
            logger.debug(f"  段落中没有加粗文本，设置加粗状态为False")
    else:
        # 如果没有收集到任何加粗状态，确保加粗状态为False
        fonts['bold'] = {False}
        logger.debug(f"  未收集到加粗状态信息，设置加粗状态为False")
    ...
    if run_italic_states:
        ...
        if italic_text_length > 0:
            fonts['italic'].add(True)
            logger.debug(f"  段落中有斜体文本，斜体文本长度: {italic_text_length}, 总文本长度: {total_text_length}")
        else:
            # 确保斜体状态为空集合，并添加False值
            fonts['italic'] = {False}
            logger.debug(f"  段落中没有斜体文本，设置斜体状态为False")
    else:
        # 如果没有收集到任何斜体状态，确保斜体状态为False
        fonts['italic'] = {False}
        logger.debug(f"  未收集到斜体状态信息，设置斜体状态为False")
                ...
                for font in root.findall('.//a:font[@script="Hans"]', namespaces):
                    if 'typeface' in font.attrib:
                        ...
                        if typeface in common_cn_fonts or any(cn_font in typeface for cn_font in common_cn_fonts):
                            ...
                            logger.debug(f"从theme1.xml提取到Hans中文字体: {typeface}")
                ...
                if not fonts['zh_family']:
                    ...
                    for ea in ea_fonts:
                        if 'typeface' in ea.attrib:
                            ...
                            if typeface in common_cn_fonts or any(cn_font in typeface for cn_font in common_cn_fonts):
                                fonts['zh_family'].add(typeface)
                                logger.debug(f"从theme1.xml提取到东亚字体: {typeface}")
                ...
                if not fonts['zh_family']:
                    ...
                    if font_scheme is not None:
                        ...
                        if default_font is not None and 'typeface' in default_font.attrib:
                            ...
                            if typeface in common_cn_fonts or any(cn_font in typeface for cn_font in common_cn_fonts):
                                fonts['zh_family'].add(typeface)
                                logger.debug(f"从fontScheme提取到Hans中文字体: {typeface}")
                ...
                for font in root.findall('.//a:font[@script="Latn"]', namespaces):
                    if 'typeface' in font.attrib:
                        ...
                        if not any(cn_font in typeface for cn_font in common_cn_fonts):
                            ...
                            logger.debug(f"从theme1.xml提取到Latn英文字体: {typeface}")
                            # 如果是双语字体，也添加到中文字体集合中
                            if typeface in bilingual_fonts or any(bi_font in typeface for bi_font in bilingual_fonts):
                                fonts['zh_family'].add(typeface)
                                logger.debug(f"将双语字体添加到中文字体集合: {typeface}")
                        else:
                            # 如果是中文字体，则只添加到中文字体集合
                            fonts['zh_family'].add(typeface)
                            logger.debug(f"从Latn标签中提取到中文字体并正确分类: {typeface}")
                ...
                if not fonts['en_family']:
                    for latin in root.findall('.//a:latin', namespaces):
                        if 'typeface' in latin.attrib:
                            ...
                            if not any(cn_font in typeface for cn_font in common_cn_fonts):
                                fonts['en_family'].add(typeface)
                                logger.debug(f"从theme1.xml提取到latin字体: {typeface}")
                                # 如果是双语字体，也添加到中文字体集合中
                                if typeface in bilingual_fonts or any(bi_font in typeface for bi_font in bilingual_fonts):
                                    fonts['zh_family'].add(typeface)
                                    logger.debug(f"将双语字体添加到中文字体集合: {typeface}")
                                ...
                                # 如果是中文字体，则只添加到中文字体集合
                                fonts['zh_family'].add(typeface)
                                logger.debug(f"从latin标签中提取到中文字体并正确分类: {typeface}")
                ...
                if not fonts['en_family']:
                    for ascii_font in root.findall('.//a:font[@script="Ascii"]', namespaces):
                        if 'typeface' in ascii_font.attrib:
                            ...
                            if not any(cn_font in typeface for cn_font in common_cn_fonts):
                                fonts['en_family'].add(typeface)
                                logger.debug(f"从theme1.xml提取到Ascii英文字体: {typeface}")
                                # 如果是双语字体，也添加到中文字体集合中
                                if typeface in bilingual_fonts or any(bi_font in typeface for bi_font in bilingual_fonts):
                                    fonts['zh_family'].add(typeface)
                                    logger.debug(f"将双语字体添加到中文字体集合: {typeface}")
                                ...
                                # 如果是中文字体，则只添加到中文字体集合
                                fonts['zh_family'].add(typeface)
                                logger.debug(f"从Ascii标签中提取到中文字体并正确分类: {typeface}")
                    ...
                    for font_type in ['a:majorFont', 'a:minorFont']:
                        ...
                        if font_elem is not None:
                            ...
                            if latin is not None and 'typeface' in latin.attrib:
                                ...
                                if not any(cn_font in typeface for cn_font in common_cn_fonts):
                                    fonts['en_family'].add(typeface)
                                    logger.debug(f"从{font_type}提取到英文字体: {typeface}")
                                    # 如果是双语字体，也添加到中文字体集合中
                                    if typeface in bilingual_fonts or any(bi_font in typeface for bi_font in bilingual_fonts):
                                        fonts['zh_family'].add(typeface)
                                        logger.debug(f"将双语字体添加到中文字体集合: {typeface}")
                                    ...
                                    # 如果是中文字体，则只添加到中文字体集合
                                    fonts['zh_family'].add(typeface)
                                    logger.debug(f"从{font_type}标签中提取到中文字体并正确分类: {typeface}")

                logger.debug(f"从theme1.xml提取的字体: 中文字体={fonts['zh_family']}, 英文字体={fonts['en_family']}")
    except Exception as e:
        logger.error(f"从theme1.xml提取字体信息时出错: {str(e)}")
...
def extract_font_info_from_xml(xml_data):
    ...
    try:
        ...
        for rFonts in root.findall('.//w:rFonts', namespaces):
            ...
            if ascii:
                ...
                if any(cn_font in ascii for cn_font in ['宋体', '黑体', '楷体', '仿宋', '华文', '微软雅黑', '等线', '方正', '思源', '苹方']):
                    ...
                    logger.debug(f"从XML中提取到中文字体并正确分类: {ascii}")
                else:
                    # 如果不是中文字体，则添加到英文字体集合
        ...
        logger.error(f"从 XML 提取字体信息时出错: {str(e)}")
        # 添加默认颜色
        fonts['color'].add('black')
        # 确保加粗和斜体设置为False
        fonts['bold'] = {False}
