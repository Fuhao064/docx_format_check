...
from backend.para_type import ParagraphManager, ParsedParaType, ParaInfo
...
class FormatEditor:
    ...
    def _apply_font_settings(self, run, font_settings: Dict, is_chinese: bool = True):
        """应用字体设置到文本运行对象"""
        # 设置字体族
    ...
    def _apply_paragraph_format(self, paragraph, format_settings: Dict):
        """应用段落格式设置"""
        # 设置行间距
        if 'line_spacing' in format_settings:
            spacing = format_settings['line_spacing']
            if spacing in self.line_spacing_map:
                if spacing == "1.5":
        ...
        if 'alignment' in format_settings:
            ...
            if alignment in self.alignment_map:
                paragraph.paragraph_format.alignment = self.alignment_map[alignment]
    ...
    def _split_text_by_language(self, text: str) -> List[Tuple[str, bool]]:
        """
        将文本按中英文分段
    ...
    def _get_openai_image_caption(self, image_path: str) -> str:
        """
        使用OpenAI API获取图片题注
    ...
    def _set_paper_format(self, doc: Document):
        """设置文档页面格式"""
    ...
    def format_document(self, doc_path: str, para_manager: ParagraphManager, output_path: Optional[str] = None):
        """
        根据配置和段落管理器格式化文档
        ...
        for para_info in para_manager.paragraphs:
            ...
            # 获取段落格式设置
            if para_type in self.config:
                format_config = self.config[para_type]
                
                # 应用段落格式
                if 'paragraph_format' in format_config:
                    self._apply_paragraph_format(paragraph, format_config['paragraph_format'])
                ...
                if 'fonts' in format_config:
                    ...
                    # 拆分中英文
                    segments = self._split_text_by_language(para_info.content)
                    
                    # 为每段添加相应的格式
                    for text, is_chinese in segments:
                        run = paragraph.add_run(text)
                        self._apply_font_settings(run, font_settings, is_chinese)
                else:
                    # 无特殊字体设置，使用默认体
            ...
            # 特殊处理图片段落
            if para_type == 'figures' and para_info.meta.get('image_path'):
                ...
                if os.path.exists(image_path):
                    ...
                    if 'paragraph_format' in self.config['figures']:
                        self._apply_paragraph_format(img_paragraph, self.config['figures']['paragraph_format'])
                    ...
                    if not caption:
                        caption = self._get_openai_image_caption(image_path)
                    
                    # 添加题注段落
                    caption_paragraph = doc.add_paragraph()
                    if 'paragraph_format' in self.config['figures']:
                        self._apply_paragraph_format(caption_paragraph, self.config['figures']['paragraph_format'])
                    
                    # 设置题注文本
                    caption_run = caption_paragraph.add_run(f"图 {para_info.meta.get('figure_number', '')} {caption}")
                    
                    # 应用题注字体格式
                    if 'caption' in self.config['figures'] and 'fonts' in self.config['figures']['caption']:
                        self._apply_font_settings(caption_run, self.config['figures']['caption']['fonts'])
    ...
    def add_figure_caption(self, doc_path: str, image_path: str, figure_number: int, caption: Optional[str] = None, 
        ...
        paragraph = doc.add_paragraph()
        if 'paragraph_format' in self.config['figures']:
            self._apply_paragraph_format(paragraph, self.config['figures']['paragraph_format'])
        ...
        # 获取题注位置设置
        position = "below"
        if 'caption' in self.config['figures'] and 'position' in self.config['figures']['caption']:
            position = self.config['figures']['caption']['position']
    ...
    def format_table_caption(self, doc_path: str, table_number: int, caption: str, output_path: Optional[str] = None):
        """为文档中的表格添加题注
        ...
        # 获取题注位置设置
        position = "above"
        if 'caption' in self.config['tables'] and 'position' in self.config['tables']['caption']:
            position = self.config['tables']['caption']['position']
        
        # 添加题注段落
        caption_paragraph = doc.add_paragraph()
        if 'paragraph_format' in self.config['tables']:
            self._apply_paragraph_format(caption_paragraph, self.config['tables']['paragraph_format'])
        ...
        # 应用题注字体格式
        if 'caption' in self.config['tables'] and 'fonts' in self.config['tables']['caption']:
            self._apply_font_settings(caption_run, self.config['tables']['caption']['fonts'])
    ...
    def generate_formatted_doc(self, para_manager: ParagraphManager, output_path: str) -> str:
        """
        ...
        for para_info in para_manager.paragraphs:
            ...
            # 处理表格类型的段落
                ...
                if isinstance(table_data, list) and len(table_data) > 0:
                    ...
                    # 应用表格样式
                    if 'caption' in self.config['tables'] and 'border' in self.config['tables']['caption']:
                        border_config = self.config['tables']['caption']['border']
                    ...
                    for i, row_data in enumerate(table_data):
                        for j, cell_data in enumerate(row_data if isinstance(row_data, list) else [row_data]):
                            ...
                            for paragraph in cell.paragraphs:
                                if 'paragraph_format' in self.config['tables']:
                                    self._apply_paragraph_format(paragraph, self.config['tables']['paragraph_format'])
                                
                                for run in paragraph.runs:
                                    if 'caption' in self.config['tables'] and 'fonts' in self.config['tables']['caption']:
                                        self._apply_font_settings(run, self.config['tables']['caption']['fonts'])
...
# 使用示例
# editor = FormatEditor("config.json")
# para_manager = ParagraphManager()
# para_manager.add_para(ParsedParaType.TITLE_ZH, "测试文档标题")
# output_path = editor.generate_formatted_doc(para_manager, "output.docx")
